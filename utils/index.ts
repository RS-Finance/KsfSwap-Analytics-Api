...
import { TOP_PAIRS, PAIRS_VOLUME_QUERY, TOKEN_BY_ADDRESS, KCS_PRICE } from "./apollo/queries";
...
export async function getKcsPrice(): Promise<any>{
  let result = await client.query({
    query: KCS_PRICE,
    fetchPolicy: 'cache-first',
  })
  return result?.data?.bundles[0]?.kcsPrice
}
